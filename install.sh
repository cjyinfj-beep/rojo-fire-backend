...
DB_PASS="rojo123"
DB_NAME="rojo_fire"
APP_DIR="/home/admin/rojo-backend"
APP_PORT=10000
EMQX_HOST="139.196.153.51"

# ---------- 0. 检查是否 root 或使用 sudo ----------
if [ "$EUID" -ne 0 ]; then
    echo "⚠️  当前不是 root，尝试用 sudo 运行..."
    sudo "$0" "$@"
    exit 0
fi
...
if [ ! -d "$APP_DIR" ]; then
    ...
    echo "   请确认代码已上传到 /home/admin/rojo-backend"
    exit 1
fi
...
directory=/home/admin/rojo-backend
command=/home/admin/rojo-backend/venv/bin/python /home/admin/rojo-backend/app.py
autostart=true
autorestart=true
user=admin
stderr_logfile=/var/log/rojo.err.log
stdout_logfile=/var/log/rojo.out.log
environment=DATABASE_URL="postgresql://rojo:rojo123@localhost:5432/rojo_fire",EMQX_HOST="139.196.153.51"
SUP_EOF
