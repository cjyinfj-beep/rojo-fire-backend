...
def get_history(device_id):
    ...
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("""
        SELECT temperature, distance, color_status, alarm, alarm_reason, created_at
        FROM sensor_history
...
def get_alarms(device_id):
    conn = get_db()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM alarms WHERE device_id = %s ORDER BY created_at DESC LIMIT 100;", (device_id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return jsonify(list(rows))
...
def send_command(device_id):
    data = request.get_json(force=True)
    command = data.get('command', 'buzzer_on')
    client = mqtt.Client()
    try:
        client.connect(EMQX_HOST, EMQX_PORT, 60)
        payload = {"device_id": device_id, "command": command, "timestamp": int(time.time())}
        client.publish(MQTT_TOPIC_PUB, json.dumps(payload))
        client.disconnect()
        return jsonify({"success": True, "command": command, "device_id": device_id})
